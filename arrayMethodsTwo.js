...
let numbers2 = numbers.map((element) => {
  return element / 2
})
...
let iCities = cities.filter((element) => {
...
let riceSumPrice = foods.filter((element) => element.tags.includes('rice')).reduce((acc, cur) => acc + cur.price, 0)

// THE TOTAL

console.log(riceSumPrice);